...
public class FeelPosLearning implements LearningEventListener {
	...
	public void run() {
		...
		for (int i = 0; i < 10000; i++) {
			//第一象限期望
			dataSet.add(new DataSetRow(new double[]{1 * nextDouble(), 1 * nextDouble()}, new double[]{1, 1}));
			//第二象限期望
			dataSet.add(new DataSetRow(new double[]{-1 * nextDouble(), 1 * nextDouble()}, new double[]{0, 1}));
			//第三象限期望
			dataSet.add(new DataSetRow(new double[]{-1 * nextDouble(), -1 * nextDouble()}, new double[]{0, 0}));
			//第四象限期望
			dataSet.add(new DataSetRow(new double[]{1 * nextDouble(), -1 * nextDouble()}, new double[]{1, 0}));
		}

		//创建两个输入，两个输出的感知机
		SimplePerceptron2 posPerceptron = new SimplePerceptron2(2);
